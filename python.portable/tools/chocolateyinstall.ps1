...
if ( $osBitness ){
  & "$toolsDir\nuget1.exe" install pythonx86 -Version $version -OutputDirectory "$toolsDir\Python" -Verbosity "quiet"
} else {
  & "$toolsDir\nuget1.exe" install python -Version $version -OutputDirectory "$toolsDir\Python" -Verbosity "quiet"
}
